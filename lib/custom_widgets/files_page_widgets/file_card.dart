...
class FileCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final openFile = () {
      Navigator.push(
        ...
        MaterialPageRoute(
          builder:
              (_) => SingleFile(
                url: state.parseGetExtPath(targetPath),
                name: name,
    ...
    final iconWidgets = <Widget>[
      Expanded(
        child: IconButton(
          onPressed: contentType == "dir" ? openDir : openFile,
          icon: Icon(Icons.zoom_out_map_rounded),
        ),
      ),
      if (!state.isRootPath())
        Expanded(child: IconButton(onPressed: rmDir, icon: Icon(Icons.delete))),
    ];
    if (contentType != "dir") {
      iconWidgets.insert(
        1,
                      ...
                      state.isConnected
                          ? showDialog(
                            ...
                            builder: (BuildContext context) {
                              return RenameRessourceDialog(index: index);
                            },
                          )
                          : showNotConnectedDialog(context);
                    },
