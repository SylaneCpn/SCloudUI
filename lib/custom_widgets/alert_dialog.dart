...
void showAlertDialogRefused(BuildContext context) {
  AlertDialog alert = AlertDialog(
    ...
    content: const Text(
      "Votre identifiant ou votre mot de passe est peut-être erroné. Veuillez réessayer.",
    ),
    actions: <Widget>[
      TextButton(
...
void showAlertDialogNetworkFail(BuildContext context) {
  AlertDialog alert = AlertDialog(
    title: const Text('Une erreur est survenue'),
    content: const Text(
      "Impossible d'accéder au serveur. Vérifiez votre connexion internet.",
    ),
    actions: <Widget>[
      TextButton(
        onPressed: () => Navigator.pop(context, 'OK'),
...
void showConfirmRmDialog(BuildContext context, int index) {
  ...
  final continueCallBack = () async {
    ...
    if (report == FetchingReport.success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            "Erreur de réseau, la ressource n'a pas été supprimée.",
          ),
        ),
      );
    }
  ...
  AlertDialog alert = AlertDialog(
    ...
    content: Text(
      "Vous êtes sur le point de supprimer la ressource : $name. Poursuivre l'opération ?",
    ),
    actions: <Widget>[
      TextButton(
      ...
      TextButton(onPressed: continueCallBack, child: const Text('Continuer')),
    ],
  );
...
void showNotConnectedDialog(BuildContext context) {
  AlertDialog alert = AlertDialog(
    ...
    content: const Text(
      "Vous devez être connecté pour pouvoir réaliser cette action.",
    ),
    actions: <Widget>[
      TextButton(
        onPressed: () => Navigator.pop(context, 'OK'),
...
void showSnackBarSuccessFileSend(BuildContext context) {
  ScaffoldMessenger.of(
    context,
  ).showSnackBar(SnackBar(content: Text("Fichier(s) envoyé(s) avec succès.")));
}
...
void showSnackBarFailFileSend(BuildContext context) {
  ScaffoldMessenger.of(context).showSnackBar(
    SnackBar(
      content: Text(
        "Erreur : Le serveur n'autorise pas la création de fichier(s) ou n'a pas pu écrire tous les fichiers.",
      ),
    ),
  );
}
...
void showSnackBarSuccessDirAdd(BuildContext context) {
  ScaffoldMessenger.of(
    context,
  ).showSnackBar(SnackBar(content: Text("Dossier créé avec succès.")));
}
...
void showSnackBarFailDirAdd(BuildContext context) {
  ScaffoldMessenger.of(context).showSnackBar(
    SnackBar(
      content: Text(
        "Erreur : Le serveur n'autorise pas la création de dossier ou n'a pas pu écrire le dossier.",
      ),
    ),
  );
}
...
void showSnackBarSuccessRename(BuildContext context) {
  ScaffoldMessenger.of(
    context,
  ).showSnackBar(SnackBar(content: Text("Ressource renommée avec succès.")));
}
...
void showSnackBarFailRename(BuildContext context) {
  ScaffoldMessenger.of(context).showSnackBar(
    SnackBar(
      content: Text(
        "Erreur : Le serveur n'autorise pas ou n'a pas pu renommer la ressource.",
      ),
    ),
  );
}
...
void showSnackBarNetworkFail(BuildContext context) {
  ScaffoldMessenger.of(context).showSnackBar(
    SnackBar(
      content: Text(
        "Une erreur réseau est survenue. Vérifier votre connexion internet et réessayez.",
      ),
    ),
  );
}
...
void showSnackBarInvalidInput(BuildContext context) {
  ScaffoldMessenger.of(context).showSnackBar(
    SnackBar(
      content: Text(
        "Un ou plusieurs fichiers contiennent un format interdit. Il n'ont pas pu être envoyés",
      ),
    ),
  );
}
